...
func (db *appdbimpl) Select_stream(requestingUser uint64, page_numb uint64) (schema.PostList, error) {
	...
	if err != nil {return photoList, err}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&photoId, &authorId, &username, &caption, &creation, &likes, &comments, &isLiked); err != nil {return photoList, err}
	...
	if err := rows.Err(); err != nil {return photoList, err}
